...
cmd="${1:?usage: intake.sh check <ISSUE-ID> [--allow-existing-pr] | queue --dry-run}"; shift

case "$cmd" in
  check)
    id="${1:?issue id}"; allow_pr="${2:-}"
    ...
    if printf '%s' "$issue" | jq -e '.labels.nodes[] | select(.name=="Security")' >/dev/null; then
      log "SECURITY-LABELED: $id is ineligible in every mode"; exit 2
    fi
    pr_url="$(printf '%s' "$issue" | jq -r '[.attachments.nodes[].url | select(test("/pull/"))][0] // empty')"
    if [ -n "$pr_url" ] && [ "$allow_pr" != "--allow-existing-pr" ]; then
