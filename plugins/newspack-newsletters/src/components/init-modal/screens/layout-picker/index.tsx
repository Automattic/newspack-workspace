...
// `DataViewsPicker` value is imported from `@wordpress/dataviews/wp` above.
import type { Field, ViewPickerGrid } from '@wordpress/dataviews';
...
const TABS: Array< { key: string; title: string; filter: ( layout: Layout ) => boolean | undefined } > = [
	{
		key: 'prebuilt',
...
function getMetaForPreview( layout?: Layout ) {
	const meta = ( layout?.meta || {} ) as LayoutPreviewMeta;
	return {
...
function renderAuthor( { item }: { item: Layout } ) {
...
function PreviewCard( { item }: { item: Layout } ) {
...
export default function LayoutPicker() {
	...
	// `activeTabKey` always matches one of the static `TABS` entries above.
	const activeTab = TABS.find( t => t.key === activeTabKey ) as ( typeof TABS )[ number ];
	...
	const view = useMemo< ViewPickerGrid >(
		() => ( {
			type: 'pickerGrid',
			mediaField: 'preview',
			titleField: 'title',
	...
	const insertLayout = ( layoutId: number | null ) => {
		const layout = find( layouts, { ID: layoutId as number } ) || {};
