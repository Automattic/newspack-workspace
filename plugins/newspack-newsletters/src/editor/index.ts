...
if ( newspack_email_editor_data.supported_social_icon_services ) {
	const supportedSocialIconServices = newspack_email_editor_data.supported_social_icon_services;
	addFilter( 'blocks.registerBlockType', 'newspack-newsletters/core-social-links', ( settings, name ) => {
		if ( 'core/social-link' === name && settings.variations ) {
			settings.variations = settings.variations.filter( ( variation: { name: string } ) =>
				supportedSocialIconServices.includes( variation.name )
			);
		}
		return settings;
	} );
...
registerPlugin( 'newspack-newsletters-plugin', {
	...
	// `icon` is optional and not `null`-typed; `undefined` renders identically (no icon).
	icon: undefined,
} );
