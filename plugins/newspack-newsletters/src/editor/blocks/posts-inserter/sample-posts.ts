...
const sampleAsset = ( name: string ) => `${ window.newspack_email_editor_data?.sample_assets_url || '' }${ name }`;

const buildSample = ( id: number, image: string, title: string, excerpt: string ) => ( {
	id: `sample-${ id }`,
