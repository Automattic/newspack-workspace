...
interface AdBlockEditProps {
	setAttributes: ( attributes: { adId?: string } ) => void;
	attributes: {
		adId?: string;
	};
}

export default function SubscribeEdit( { setAttributes, attributes: { adId } }: AdBlockEditProps ) {
	...
	const { placements } = useSelect( select => {
		return {
			placements: (
			 ...
			 ).getEntityRecords( 'taxonomy', 'newspack_nl_ad_placement', { per_page: -1, hide_empty: false } ),
		};
	} );
