...
/** The ad post's `meta` bag, as read from `getEditedPostAttribute( 'meta' )`. */
interface AdMeta {
	price?: string;
...
function AdEdit() {
	const { status, price, startDate, expiryDate, insertionStrategy, positionInContent, positionBlockCount } = useSelect( select => {
		...
		const meta = getEditedPostAttribute( 'meta' ) as AdMeta;
		return {
	...
	const { placements, placement } = useSelect( select => {
		// `getEntityRecords` can return `null` while a request is in flight, but callers
		// below (the `placements.find(...)` at the "new placement" keydown handler) assume
		// an array, matching the original JS; cast to the shape the code relies on.
		const { getEntityRecords } = select( 'core' ) as {
			getEntityRecords: ( kind: string, name: string, query: Record< string, unknown > ) => Placement[];
		};
		const { getEditedPostAttribute } = select( 'core/editor' ) as {
			getEditedPostAttribute: ( attribute: string ) => unknown;
		};
		return {
			placements: getEntityRecords( 'taxonomy', 'newspack_nl_ad_placement', {
				per_page: -1,
				hide_empty: false,
			} ),
			placement: ( getEditedPostAttribute( 'ad_placement' ) as string[] | string | undefined ) || '',
		};
	} );
	...
	let noticeProps: { children: ReactNode; status: 'warning' | 'info' } | undefined;
		...
		<Fragment>
			<PluginDocumentSettingPanel name="newsletters-ads-settings-panel" title={ __( 'Ad settings', 'newspack-newsletters' ) }>
				...
				<SelectControl
					...
					onChange={ ( insertion_strategy: string ) => editPost( { meta: { insertion_strategy } } ) }
					options={ [
						{
							value: 'percentage',
								...
								onKeyDown={ async ev => {
									if ( ev.key === 'Enter' ) {
										...
										const existingPlacement = placements.find(
											( p: Placement ) => p.name.toLowerCase().trim() === newPlacementName.toLowerCase().trim()
										);
