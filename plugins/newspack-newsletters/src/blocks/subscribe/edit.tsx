...
/** A single subscription list's config, keyed by list ID in the lists config. */
interface ListConfigItem {
	title: string;
	description?: string;
}

/** Attributes of the Subscribe block (see block.json). */
interface SubscribeAttributes {
	displayInputLabels: boolean;
	placeholder: string;
	emailLabel: string;
	displayNameField: boolean;
	displayLastNameField: boolean;
...
interface SubscribeEditProps {
	setAttributes: ( attributes: Partial< SubscribeAttributes > ) => void;
...
export default function SubscribeEdit( {
	...
	const onChangeBackgroundColor = ( newBackgroundColor: string ) => {
							...
							<form onSubmit={ ev => ev.preventDefault() }>
									...
									<div className="newspack-newsletters-name-input">
										<div className="newspack-newsletters-name-input-item">
											<label>
												{ displayInputLabels && (
													<RichText
														onChange={ ( value: string ) => setAttributes( { nameLabel: value } ) }
										...
										{ displayLastNameField && (
											<div className="newspack-newsletters-name-input-item">
												<label>
													{ displayInputLabels && (
														<RichText
															onChange={ ( value: string ) => setAttributes( { lastNameLabel: value } ) }
								...
								<div className="newspack-newsletters-email-input">
									<label>
										{ displayInputLabels && (
											<RichText
												onChange={ ( value: string ) => setAttributes( { emailLabel: value } ) }
						...
						{ editedState === 'success' && (
							<div className="newspack-newsletters-subscribe__response">
								...
								<div className="newspack-newsletters-subscribe__message">
									<RichText
										onChange={ ( value: string ) => setAttributes( { successMessage: value } ) }
