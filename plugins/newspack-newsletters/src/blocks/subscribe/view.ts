...
/** Server-shaped metadata returned with a subscribe response. */
...
/** The subscribe form element, augmented with the runtime `endFlow` handler and the `npe` named field. */
interface SubscribeForm extends HTMLFormElement {
	endFlow: ( message: string, status?: number, wasSubscribed?: boolean, metadata?: SubscribeMetadata ) => void;
	npe?: HTMLInputElement;
}
...
domReady( function () {
	...
	document.querySelectorAll( '.newspack-newsletters-subscribe' ).forEach( container => {
		const form = container.querySelector( 'form' ) as SubscribeForm | null;
		if ( ! form ) {
			return;
		}
		const responseContainer = container.querySelector( '.newspack-newsletters-subscribe__response' ) as HTMLElement;
		const messageContainer = container.querySelector( '.newspack-newsletters-subscribe__message' ) as HTMLElement;
		...
		form.endFlow = ( message, status = 500, wasSubscribed = false, metadata = {} ) => {
			container.setAttribute( 'data-status', String( status ) );
			const messageNode = document.createElement( 'p' );
			emailInput.removeAttribute( 'disabled' );
			submit.removeChild( spinner );
			submit.removeAttribute( 'disabled' );
			form.classList.remove( 'in-progress' );
			messageNode.innerHTML = wasSubscribed ? ( container.getAttribute( 'data-success-message' ) as string ) : message;
			...
			if ( status === 200 ) {
				...
				const baseActivity: Record< string, unknown > = { email: emailInput.value };
