...
function useQuickEditCategories(): Term[] {
...
interface AdsQuickEditPanelProps {
	item: PostItem;
	advertisers: Term[];
	placements: Term[];
	onClose: () => void;
...
export default function AdsQuickEditPanel( { item, advertisers, placements, onClose, onSaved }: AdsQuickEditPanelProps ): ReactElement {
	...
	const [ advertiserSelections, setAdvertiserSelections ] = useState< TermSelection[] >( initialAdvertiserSelections );
	const [ placementSelections, setPlacementSelections ] = useState< TermSelection[] >( initialPlacementSelections );
	const [ categorySelections, setCategorySelections ] = useState< TermSelection[] >( initialCategorySelections );
	...
	const validateAgainst = ( labels: string[] ) => {
		const lower = new Set( labels.map( l => l.toLowerCase() ) );
		return ( token: string ): boolean => lower.has( String( token ).toLowerCase() );
	};
	...
	const handleSave = async () => {
		...
		try {
			...
			setIsBusy( false );
			notifyError( errorMessage( error ) || __( 'Could not update ad. Please try again.', 'newspack-newsletters' ) );
		}
	};
