...
const ADVERTISERS: Term[] = [ { id: 10, name: 'Acme' } ];
const PLACEMENTS: Term[] = [ { id: 20, name: 'Header' } ];

interface RenderPanelExtra {
	onClose?: () => void;
	onSaved?: () => void;
}

const renderPanel = ( item: PostItem, extra: RenderPanelExtra = {} ) =>
	render(
		<AdsQuickEditPanel
			item={ item }
...
const makeItem = ( status: string ): PostItem => ( { id: 42, status, title: { raw: 'Summer sale' }, meta: {} } );

const postCall = () => mockApiFetch.mock.calls.find( call => call[ 0 ]?.method === 'POST' )?.[ 0 ];
