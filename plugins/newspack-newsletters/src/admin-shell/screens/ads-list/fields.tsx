...
import type { Field, FieldRenderProps, PostItem, StatusField, Term } from '../../types';

const STATUS_KIND_ICONS: Record< string, ReactElement > = {
	active: published,
	scheduled,
	expired: notAllowed,
	draft: drafts,
...
const formatTimestampAsDate = ( timestamp?: number ): string => {
	if ( ! timestamp ) {
		return '';
	}
...
const editUrl = ( item: PostItem ): string => `${ getAdminUrl() }post.php?post=${ item.id }&action=edit`;

const getTitle = ( item: PostItem ): string => item?.title?.raw ?? item?.title?.rendered ?? '';

const renderTitle = ( { item }: FieldRenderProps ) => {
...
const renderStatus = ( { item }: FieldRenderProps ) => {
	const status: StatusField = item?.newspack_newsletters_ad_status || {};
...
const renderStartDate = ( { item }: FieldRenderProps ) => formatDate( item?.meta?.start_date );
const renderExpiryDate = ( { item }: FieldRenderProps ) => formatDate( item?.meta?.expiry_date );

const renderImpressions = ( { item }: FieldRenderProps ) => String( item?.meta?.tracking_impressions ?? 0 );
const renderClicks = ( { item }: FieldRenderProps ) => String( item?.meta?.tracking_clicks ?? 0 );
const renderPrice = ( { item }: FieldRenderProps ) => {
...
const renderDate = ( { item }: FieldRenderProps ) => formatPostDate( item );
...
export function getFields( { advertisers = [], placements = [] }: GetFieldsArgs = {} ): Field< PostItem >[] {
