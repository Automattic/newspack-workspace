...
// `buildQueryParams` deliberately returns `Record< string, unknown >` — the
// REST params it produces are dynamically keyed. This narrows a value the
// tests know to be a string, throwing (not silently coercing) if that
...
describe( 'ads buildQueryParams', () => {
	...
	it( 'defaults to writable statuses (no trash) when no kind filter is set', () => {
		...
		expect( str( status ).split( ',' ) ).toEqual( expect.arrayContaining( [ 'publish', 'private', 'future', 'draft', 'pending' ] ) );
		expect( str( status ).split( ',' ) ).not.toContain( 'trash' );
	...
	it( 'includes future in the default status set so WP-scheduled ads stay visible', () => {
		...
		expect( str( status ).split( ',' ) ).toContain( 'future' );
	} );

	it( 'includes auto-draft so a post-new + back row stays visible', () => {
		const { status } = buildQueryParams( {} );
		expect( str( status ).split( ',' ) ).toContain( 'auto-draft' );
	} );
	...
	it( 'joins multiple kinds with commas', () => {
		...
		expect( str( params.newspack_newsletters_ad_status ).split( ',' ) ).toEqual( [ 'active', 'scheduled' ] );
	} );

	it( 'maps advertiser filter to the REST taxonomy param', () => {
		const params = buildQueryParams( {
			filters: [ { field: 'advertiser', operator: 'isAny', value: [ 7, 12 ] } ],
		} );
	...
	it( 'maps ad_placement filter to the taxonomy REST base, not the taxonomy slug', () => {
		...
		const params = buildQueryParams( {
			filters: [ { field: 'ad_placement', operator: 'isAny', value: [ 3 ] } ],
		} );
	...
	it( 'ignores unknown filter fields silently', () => {
		const params = buildQueryParams( {
			filters: [ { field: 'wat', operator: 'isAny', value: 'nope' } ],
		} );
