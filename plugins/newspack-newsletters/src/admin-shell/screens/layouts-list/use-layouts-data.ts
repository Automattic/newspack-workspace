...
function buildPath( view: QueryView | null ): string {
	if ( ! view ) {
		return '';
	}
...
/**
 * @param view        DataViews view state, or `null` to defer the fetch.
 * @param mutationKey Bump from the parent to force a refetch after a mutation.
 * @return Hook state.
 */
export default function useLayoutsData( view: QueryView | null, mutationKey = 0 ): CollectionData< LayoutItem > {
	return useCollectionData< LayoutItem >( {
		path: buildPath( view ),
		mutationKey,
