...
import type { LayoutItem, RawPrebuiltLayout } from './types';

interface UsePrebuiltLayoutsResult {
	layouts: LayoutItem[];
	isLoading: boolean;
}

export default function usePrebuiltLayouts(): UsePrebuiltLayoutsResult {
	...
	useEffect( () => {
		...
		apiFetch< RawPrebuiltLayout[] >( { path: '/newspack-newsletters/v1/layouts?defaults_only=1' } )
			.then( items => {
				...
				const prebuilts: LayoutItem[] = items
