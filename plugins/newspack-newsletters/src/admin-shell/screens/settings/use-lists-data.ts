...
interface UseListsData {
	lists: SubscriptionListRow[];
	isLoading: boolean;
	error: unknown;
	reload: () => Promise< void >;
	patchList: ( dbId: number, patch: Partial< SubscriptionListRow > ) => Promise< SubscriptionListRow >;
}

export default function useListsData(): UseListsData {
	const [ lists, setLists ] = useState< SubscriptionListRow[] >( [] );
	const [ isLoading, setIsLoading ] = useState( true );
	const [ error, setError ] = useState< unknown >( null );
	const sequencesRef = useRef< Map< number, number > >( new Map() );
	const queuesRef = useRef< Map< number, Promise< SubscriptionListRow > > >( new Map() );
	const confirmedRef = useRef< Map< number, SubscriptionListRow > >( new Map() );

	const load = useCallback( async () => {
		setIsLoading( true );
		setError( null );
		try {
			const response = await apiFetch< SubscriptionListRow[] >( { path: LISTS_PATH } );
			const next = Array.isArray( response ) ? response : [];
			setLists( next );
			const confirmed = new Map< number, SubscriptionListRow >();
			next.forEach( row => {
	...
	const patchList = useCallback( ( dbId: number, patch: Partial< SubscriptionListRow > ): Promise< SubscriptionListRow > => {
		...
		const next: Promise< SubscriptionListRow > = previous
			.catch( () => {} )
			.then( async () => {
				try {
					const response = await apiFetch< SubscriptionListRow >( {
