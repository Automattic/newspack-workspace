...
export default function SettingsScreen(): ReactElement {
	...
	const [ pendingOptions, setPendingOptions ] = useState< Record< string, string | boolean > >( {} );
	...
	const clearOptionKeys = useCallback( ( keys: string[] ) => {
	...
	const handleSaveProvider = useCallback( async () => {
		...
		const payload: { provider: { slug: string; credentials?: Record< string, string > } } = { provider: { slug: pendingSlug } };
		...
		try {
			...
			notifyError( errorMessage( err ) || __( 'Could not save settings. Check the credentials and try again.', 'newspack-newsletters' ) );
		} finally {
			setIsSaving( false );
		}
	...
	const saveOptionsSubset = useCallback(
		async ( keys: string[], successMessage: string ) => {
			...
			try {
				...
				notifyError( errorMessage( err ) || __( 'Could not save settings.', 'newspack-newsletters' ) );
			} finally {
				setIsSaving( false );
			}
		},
		...
		return (
			<VStack spacing={ 12 } className="newspack-newsletters-settings">
				<Notice status="error" isDismissible={ false }>
					{ errorMessage( error ) || __( 'Could not load settings. Refresh the page to try again.', 'newspack-newsletters' ) }
				</Notice>
			</VStack>
		);
	}
