...
interface ListsSectionProps {
	lists?: SubscriptionListRow[];
	isLoading: boolean;
	error: unknown;
	canAddLocal: boolean;
	onPatchList: ( dbId: number, patch: Partial< SubscriptionListRow > ) => Promise< SubscriptionListRow >;
	onLocalListChanged?: () => void;
}

// `null` = closed, `'add'` = create modal, `{ list, kind }` = edit modal pre-populated.
type ModalState = null | 'add' | { list: SubscriptionListRow; kind: ListKind };

export default function ListsSection( { lists, isLoading, error, canAddLocal, onPatchList, onLocalListChanged }: ListsSectionProps ): ReactElement {
	const [ modalState, setModalState ] = useState< ModalState >( null );
	const [ deletingId, setDeletingId ] = useState< number | null >( null );
	const [ pendingDelete, setPendingDelete ] = useState< SubscriptionListRow | null >( null );
	...
	const confirmDelete = async () => {
		...
		try {
			...
			notifyError( errorMessage( err ) || __( 'Could not delete the local list.', 'newspack-newsletters' ) );
			setPendingDelete( null );
		} finally {
			setDeletingId( null );
		}
	...
	const handleToggleActive = async ( list: SubscriptionListRow, next: boolean ) => {
		try {
			await onPatchList( list.db_id, { active: next } );
		} catch ( err ) {
			notifyError( errorMessage( err ) || __( 'Could not update the subscription list.', 'newspack-newsletters' ) );
		}
	};
			...
			<Card>
				...
				<CardBody>
					<Notice status="error" isDismissible={ false }>
						{ errorMessage( error ) || __( 'Could not load lists from the configured provider.', 'newspack-newsletters' ) }
					</Notice>
				</CardBody>
			</Card>
		);
