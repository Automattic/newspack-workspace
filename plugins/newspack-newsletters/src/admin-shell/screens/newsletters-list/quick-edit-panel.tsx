...
import type { PostItem, Term } from '../../types';
...
function useQuickEditOptions(): QuickEditOptions {
	const [ options, setOptions ] = useState< QuickEditOptions >( { categories: [], tags: [] } );
...
interface NewslettersQuickEditPanelProps {
	item: PostItem;
	onClose: () => void;
	onSaved: () => void;
}

export default function NewslettersQuickEditPanel( { item, onClose, onSaved }: NewslettersQuickEditPanelProps ): ReactElement {
	...
	const validateAgainst = ( names: string[] ) => {
	...
	const handleSave = async () => {
		...
		try {
			...
			setIsBusy( false );
			notifyError( errorMessage( error ) || __( 'Could not update newsletter. Please try again.', 'newspack-newsletters' ) );
		}
	};
