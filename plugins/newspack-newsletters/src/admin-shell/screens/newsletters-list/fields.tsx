...
import type { Field, FieldRenderProps, PostItem, StatusField } from '../../types';
import type { FilterElements } from './use-filter-elements';
...
const renderStatus = ( { item }: FieldRenderProps ) => {
	const status: StatusField = item?.newspack_newsletters_status || {};
...
const renderSendDate = ( { item }: FieldRenderProps ) => {
	const status: StatusField = item?.newspack_newsletters_status || {};
	const ts = status.sent_at || status.scheduled_at;
	return ts ? formatDate( ts ) : '';
};

const renderSendList = ( { item }: FieldRenderProps ) => {
...
const renderAuthor = ( { item }: FieldRenderProps ) => {
...
const renderPublicPage = ( { item }: FieldRenderProps ) => {
...
export function getFields( { authors = [], categories = [], tags = [], sendLists = [] }: Partial< FilterElements > = {} ): Field< PostItem >[] {
