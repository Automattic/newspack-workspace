...
const setIsPublic = ( id: number, isPublic: boolean ) =>
	apiFetch( {
		path: `${ POSTS_PATH }/${ id }`,
		method: 'POST',
...
const isMakePublicEligible = ( item: PostItem ): boolean => ! isTrashed( item ) && ! item?.meta?.is_public;
const isMakeNonPublicEligible = ( item: PostItem ): boolean => ! isTrashed( item ) && !! item?.meta?.is_public;

const visibilitySuccess = ( n: number ): string =>
	sprintf(
...
const visibilityFailure = ( n: number ): string =>
	sprintf(
...
export function getActions( { refresh, openQuickEdit }: GetActionsArgs ): ListAction[] {
	...
	const makePublicAction: ListAction = {
		id: 'make-public',
	...
	const makeNonPublicAction: ListAction = {
		id: 'make-non-public',
