...
// Narrows to the `RenderModal` variant, for the Rename action's modal checks.
function modalOf( action: ListAction ) {
	if ( 'RenderModal' in action ) {
		return action;
	}
	throw new Error( `Action "${ action.id }" has no RenderModal` );
}
...
describe( 'newsletters list actions', () => {
	...
	const draftRow: PostItem = { id: 1, status: 'draft', meta: { is_public: false }, link: '' };
	const sentPublicRow: PostItem = {
		id: 2,
		status: 'publish',
		meta: { is_public: true },
		link: 'https://example.test/n/2',
	};
	const trashedRow: PostItem = { id: 3, status: 'trash', meta: { is_public: false }, link: '' };
	...
	it( 'Quick Edit delegates to openQuickEdit with the row item', () => {
		...
		callbackOf( byId( 'quick-edit' ) )( [ draftRow ], { registry: {} } );
	...
	it( 'Rename opens a medium modal and is eligible on non-trashed rows only', () => {
		const action = modalOf( byId( 'rename' ) );
	...
	describe( 'bulk callbacks filter to eligible rows before mutating', () => {
		...
		it( 'restore only PATCHes trashed rows out of a mixed selection', async () => {
			...
			await callbackOf( action )( selection, { registry: {} } );

			expect( mockApiFetch ).toHaveBeenCalledTimes( 1 );
			expect( mockApiFetch.mock.calls[ 0 ][ 0 ].path ).toContain( `/${ trashedRow.id }` );
			expect( mockApiFetch.mock.calls[ 0 ][ 0 ].method ).toBe( 'POST' );
			expect( mockApiFetch.mock.calls[ 0 ][ 0 ].data ).toEqual( { status: 'draft' } );
		} );
		...
		it( 'make-public only PATCHes non-trashed, currently-non-public rows', async () => {
			...
			expect( mockApiFetch ).toHaveBeenCalledTimes( 1 );
			expect( mockApiFetch.mock.calls[ 0 ][ 0 ].path ).toContain( `/${ draftRow.id }` );
			expect( mockApiFetch.mock.calls[ 0 ][ 0 ].data ).toEqual( { meta: { is_public: true } } );
		} );
		...
		it( 'make-non-public only PATCHes non-trashed, currently-public rows', async () => {
			...
			expect( mockApiFetch ).toHaveBeenCalledTimes( 1 );
			expect( mockApiFetch.mock.calls[ 0 ][ 0 ].path ).toContain( `/${ sentPublicRow.id }` );
			expect( mockApiFetch.mock.calls[ 0 ][ 0 ].data ).toEqual( { meta: { is_public: false } } );
		} );
		...
		it( 'no-eligible selection does not call apiFetch and does not refresh', async () => {
			...
			await callbackOf( byId( 'restore' ) )( [ draftRow, sentPublicRow ], { registry: {} } );
