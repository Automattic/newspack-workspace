...
/**
 * A `TreeSelect` node — matches `@wordpress/components`' `Tree` shape
 * (`{ id, name, children }`), which isn't exported as a named type from
 * the package root.
 */
interface AdvertiserTreeNode {
	id: string;
	name: string;
 ...
 * just as broken).
 *
 * @param advertisers Flat list from the REST collection.
 * @param excludeId   Optional id to omit (and its descendants).
 * @return Tree rooted at top-level (`parent === 0`).
 */
export function buildAdvertiserTree( advertisers: Term[] | null | undefined, excludeId: number | null = null ): AdvertiserTreeNode[] {
	...
	const byParent = new Map< number | undefined, Term[] >();
	...
	if ( excludeId ) {
		...
		const stack: number[] = [ excludeId ];
		while ( stack.length > 0 ) {
			const current = stack.pop();
			const children = byParent.get( current ) || [];
			for ( const child of children ) {
				if ( typeof child.id === 'number' && ! excluded.has( child.id ) ) {
	...
	const buildChildren = ( parentId: number | undefined ): AdvertiserTreeNode[] =>
...
interface AdvertiserModalProps {
	/** Existing term to edit. Null/undefined for create. */
	advertiser?: Term | null;
	/** Full flat list from the REST collection. */
	advertisers?: Term[];
...
export default function AdvertiserModal( { advertiser = null, advertisers = [], onClose, onSaved }: AdvertiserModalProps ): ReactElement {
	...
	const tree = useMemo( () => buildAdvertiserTree( advertisers, isEdit ? advertiser?.id ?? null : null ), [ advertisers, isEdit, advertiser?.id ] );
	...
	const submit = async ( event: FormEvent ) => {
		...
		try {
			...
			// swallows a failure.
			setError( errorMessage( err ) || __( 'Failed to save advertiser. Please try again.', 'newspack-newsletters' ) );
			setIsBusy( false );
		}
	};
