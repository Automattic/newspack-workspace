...
interface HeaderActionsAPI {
	upsert: ( ownerId: string, actions: HeaderAction[] ) => void;
	remove: ( ownerId: string ) => void;
}

interface HeaderActionsRegistry {
	ownerOrder: string[];
	actionsByOwner: Record< string, HeaderAction[] >;
}
...
const HeaderActionsAPIContext = createContext< HeaderActionsAPI | null >( null );
const HeaderActionsValueContext = createContext< HeaderAction[] >( [] );

export function HeaderActionsProvider( { children }: { children: ReactNode } ): ReactElement {
	const [ registry, setRegistry ] = useState< HeaderActionsRegistry >( () => ( {
		ownerOrder: [],
		actionsByOwner: {},
	} ) );

	const upsert = useCallback(
		( ownerId: string, actions: HeaderAction[] ) =>
			setRegistry( prev => {
 ...
 */
export function useHeaderActionsValue(): HeaderAction[] {
 ...
 *
 * @param actions Memoised array of action descriptors.
 */
