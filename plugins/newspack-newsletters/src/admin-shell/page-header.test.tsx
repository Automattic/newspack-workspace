...
const Harness = ( { actions }: { actions: HeaderAction[] } ) => {
...
describe( 'PageHeader', () => {
	...
	it( 'fires onClick handlers when a primary action is activated', () => {
		...
		const actions: HeaderAction[] = [ { type: 'primary', label: 'Add new', onClick } ];
		render( withProvider( <Harness actions={ actions } /> ) );
	...
	it( 'clears the actions when the registering component unmounts', () => {
		const StableHarness = () => {
			const actions = useMemo< HeaderAction[] >( () => [ { type: 'primary', label: 'Add new' } ], [] );
			return <Harness actions={ actions } />;
		};
		const ConditionalHarness = ( { mounted }: { mounted?: boolean } ) => ( mounted ? <StableHarness /> : null );
	...
	it( 'restores a still-mounted earlier registration when an overlapping later one unmounts', () => {
		...
		// ScreenB unmounts.
		const ScreenA = () => {
			const actions = useMemo< HeaderAction[] >( () => [ { type: 'primary', label: 'Action A' } ], [] );
			useHeaderActions( actions );
			return null;
		};
		const ScreenB = () => {
			const actions = useMemo< HeaderAction[] >( () => [ { type: 'primary', label: 'Action B' } ], [] );
			useHeaderActions( actions );
			return null;
		};

		const Switcher = ( { showB }: { showB?: boolean } ) => (
			<>
				<ScreenA />
				{ showB && <ScreenB /> }
			</>
	...
	it( 'is a no-op when used outside a provider', () => {
		...
		const actions: HeaderAction[] = [ { type: 'primary', label: 'Add new' } ];
		expect( () => {
	...
	it( 'propagates the latest onClick closure when only the handler reference changes', () => {
		...
		const StableHarness = ( { tag }: { tag: string } ) => {
	...
	it( "portals into newspack-plugin's admin-header strip when present", () => {
		...
		const actions: HeaderAction[] = [ { type: 'primary', label: 'Add new newsletter' } ];
		render( withProvider( <Harness actions={ actions } /> ) );
