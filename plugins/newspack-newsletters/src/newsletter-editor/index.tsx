...
interface NewsletterEditProps {
	apiFetchWithErrorHandling: ( params: { path: string; method?: string } ) => Promise< { valid: boolean; auth_url: string } >;
	setInFlightForAsync: ( value?: boolean ) => void;
	inFlight?: boolean;
}

function NewsletterEdit( { apiFetchWithErrorHandling, setInFlightForAsync, inFlight }: NewsletterEditProps ) {
	const { layoutId, postId } = useSelect( select => {
		const { getCurrentPostId, getEditedPostAttribute } = select( 'core/editor' ) as {
	...
	// `is_service_provider_configured` is typed `boolean` in the shared window-global
	// declaration (src/newspack-newsletters-globals.d.ts), but is compared against the
	// string '1' here (and typed as `string` locally in sidebar/send-to.tsx) — the
	// runtime value appears to actually be a string; widened here to match.
	const isServiceProviderConfigured = window?.newspack_newsletters_data?.is_service_provider_configured as string | boolean | undefined;
	...
	const [ shouldDisplaySettings, setShouldDisplaySettings ] = useState( ! isLayoutEditor() && isServiceProviderConfigured !== '1' );
	...
	useEffect( () => {
		if ( newsletterDataError ) {
			const dataError = newsletterDataError as NewsletterDataError;
			createNotice( 'error', dataError?.message || __( 'Error communicating with service provider.', 'newspack-newsletters' ), {
