...
2. Opts the CPT into the editor **only when `Feature_Flag::is_enabled()`** (and re-asserts Newspack's canonical CPT args at `init:11`, since the package re-registers post types at `init:10`).
...
**Why the boot is unconditional but the CPT opt-in is flag-gated.** The boot itself runs for both flag states (one boot path; it must run before `init` so the package's own `init` callbacks register in time, and the WC-engine tests call `init()` directly regardless of flag). What is gated is the **CPT opt-in** in `add_post_type()`. That matters because the package's `Email_Editor::initialize()` registers a `single_template` filter (`load_email_preview_template`) that only takes over post types present in `woocommerce_email_editor_post_types`. Left ungated, a *public* newsletter (one set for both email and web) rendered on the **front end** through the package's email-preview template — full email HTML, email-only blocks shown, no theme wrapper — instead of the theme's standard single template. Gating the opt-in keeps a flag-off site on the legacy (MJML) behavior, where public newsletters render normally in the theme and `Newspack_Newsletters::remove_visibility_hidden_block()` hides email-only blocks. The editor takeover in `Newspack_Newsletters_Editor` is separately flag-gated; the opt-in gate closes the front-end path it never covered. The flag is read lazily inside `add_post_type()` (on `init`), so every flag source — option, constant, filter — is honored regardless of registration timing. With the flag off the boot still registers only additive, unused-until-render machinery (e.g. the `block_type_metadata_settings` filter).
