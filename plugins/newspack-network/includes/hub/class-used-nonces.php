...
class Used_Nonces {
	...
	public static function release( $nonce ) {
		global $wpdb;
		$table = self::get_table_name();
		$wpdb->delete( $table, [ 'nonce' => $nonce ], [ '%s' ] ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	}
	...
	protected static function maybe_create_table() {
		...
		// so it is stored as-is under a UNIQUE key — the key is the atomic claim.
		// `created_at` is kept for diagnostics only; nothing prunes by it (see the
		// class docblock on indefinite retention), so it carries no index.
