...
/**
 * Component for rendering onboarding controls for profile collections.
 *
...
export const ProfileOnboardingControls = () => {
	...
	const handleCreateProfileCollection = async () => {
		...
		try {
			createInfoNotice(
				__( 'Creating profile collection…', 'newspack-profiles' ),
				{ type: 'snackbar', id: creatingNoticeId }
			);
			...
			console.error( 'Failed to create profile collection', error );
	...
	const handleUpdateProfileCollection = async () => {
		...
		try {
			...
			createInfoNotice(
				__( 'Updating profile collection…', 'newspack-profiles' ),
				{ type: 'snackbar', id: updatingNoticeId }
			);
			...
			createSuccessNotice(
				__( 'Profile collection updated.', 'newspack-profiles' ),
				{ type: 'snackbar' }
			);
		} catch ( error ) {
			// eslint-disable-next-line no-console
			console.error( 'Failed to update profile collection', error );

			removeNotice( updatingNoticeId );

			createErrorNotice(
				__(
					'Failed to update profile collection.',
					'newspack-profiles'
				),
				{ type: 'snackbar' }
			);
	...
	return (
		<div className="px-6 py-4 mt-4 border-t border-gray-300 flex gap-10 justify-between sticky bottom-0 bg-white z-4">
			{ controls.map( ( step ) => (
				<Button
					key={ step.key }
