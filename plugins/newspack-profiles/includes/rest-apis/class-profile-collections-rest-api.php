<?php
/**
 * REST API for profile collections.
 *
...
/**
 * Profile_Collections_Rest_Api class to handle REST API endpoints for profile collections.
 */
class Profile_Collections_Rest_Api {
	...
	/**
	 * Data source for profile collections.
	 *
	...
	/**
	 * Get all profile collections.
	 *
	 * @return WP_REST_Response|WP_Error The REST response containing profile collections or WP_Error on failure.
	 */
	...
	/**
	 * Add a new profile collection.
	 *
	 * @param WP_REST_Request $request The REST request.
	 *
	 * @return WP_REST_Response|WP_Error The REST response after adding the profile collection or WP_Error on failure.
	 */
	public function add( WP_REST_Request $request ): WP_REST_Response|WP_Error {
		...
		if ( ! empty( $existing_collection ) ) {
			return rest_ensure_response(
				new WP_Error(
					...
					'Profile collection already exists.',
					array( 'status' => 400 )
				)
			);
		}
		...
		if ( 'wpdb' === $collection['dataSource']['type'] ) {
			return rest_ensure_response(
				new WP_Error(
					'invalid_data_source',
					'Cannot create a new profile collection with wpdb data source. This data source type is reserved for internal use.',
					array( 'status' => 400 )
				)
			);
		}
		...
		if ( empty( $pages ) ) {
			return rest_ensure_response(
				new WP_Error(
					...
					'Failed to create profile collection pages.',
					array( 'status' => 500 )
				)
			);
		}
		...
		/**
		 * Update rewrite rules in next init cycle to include new profile collection rewrite rules.
		 */
	...
	/**
	 * Update a profile collection.
	 *
	 * @param WP_REST_Request $request The REST request.
	 *
	 * @return WP_REST_Response|WP_Error The REST response after updating the profile collection or WP_Error on failure.
	 */
	 ...
	 * @param WP_REST_Request $request The REST request.
	 *
	 * @return WP_REST_Response|WP_Error The REST response after deleting the profile collection or WP_Error on failure.
	 */
	...
	/**
	 * Disconnect the remote data source for a profile collection.
	 *
	 * @param WP_REST_Request $request The REST request.
	 *
	...
	/**
	 * Update the status of a profile collection.
	 *
	 * @param WP_REST_Request $request The REST request.
	 *
