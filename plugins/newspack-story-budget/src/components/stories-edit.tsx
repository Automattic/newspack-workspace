...
const EditFieldPanel = ( { field, value, disabled = false, onChange, onToggle, initialOpen = false }: EditFieldPanelProps ) => {
...
const FieldControl = ( { field, story, onChange, disabled }: FieldControlProps ) => {
	const fieldProps = useStoryField( story.id, field.slug ) as Field | null;
	...
	return (
		<VStack spacing={ 2 }>
			...
			<StoryFieldControl
				field={ fieldProps }
				value={ ( story[ fieldProps.slug ] as FieldValue ) || EMPTY_STRING }
				onChange={ onChange }
				disabled={ disabled }
			/>
		</VStack>
...
const getItemsTitle = ( items: Story[] ): string | number => {
	...
	return sprintf(
		...
		__( '%1$s, and %2$s', 'newspack-story-budget' ),
		titles.join( ', ' ),
		( lastTitle as string | number ).toString()
	);
};

export interface StoriesEditProps {
	items: Story[];
	closeModal: () => void;
	onActionPerformed?: ( items: Story[] ) => void;
}

export default ( { items, closeModal, onActionPerformed }: StoriesEditProps ) => {
	const isBulk = items.length > 1;

	const { saveError, isSavingStories } = useSelect( select => {
		const selectors = select( storeNamespace ) as StoryBudgetSelectors;
		return {
			saveError: selectors.getSaveStoriesError(),
			isSavingStories: selectors.isSavingStories(),
		};
	} );
	...
	const [ editedStory, setEditedStory ] = useState< Record< string, unknown > >( isBulk ? {} : items[ 0 ] );

	const [ editedFields, setEditedFields ] = useState< string[] >(
		isBulk ? [] : fields.filter( field => field.is_editable ).map( field => field.slug )
	);
	...
	// eslint-disable-next-line react-hooks/rules-of-hooks -- pre-existing: these hooks run after the early return above.
	useEffect( () => {
	...
	const bulkFields = fields.filter( field => ! nonBulkTypes.includes( field.type as string ) && field.is_editable && field.slug !== 'status' );

	const handleFieldToggle = ( field: Field ) => ( open: boolean ) => {
					...
					<Panel>
						{ bulkFields.map( field => (
							<EditFieldPanel
								key={ field.slug }
								field={ field }
								value={ ( editedStory[ field.slug ] as FieldValue ) || EMPTY_STRING }
							...
							<HStack key={ field } expanded className="newspack-story-budget__field-row">
								...
								<Text className="newspack-story-budget__field">
									<Text className="newspack-story-budget__field__value">
										{ /* `utils/fields`' `Field` isn't identical to this subtree's local `Field`; cast at this cross-module boundary. */ }
										{ ( getDisplayValue(
											fields.find( f => f.slug === field )! as Parameters< typeof getDisplayValue >[ 0 ],
											editedStory[ field ]
										) as string | null ) || (
