...
import type { Field, Story, StoryBudgetSelectors } from './types';
...
export default ( { storyId, onCancel }: StoryProps ) => {
	const { isLoadingStory, canEditStory, storyError } = useSelect( select => {
		...
		return {
			...
			storyError: selectors.getStoryError( storyId ),
		};
	} );
	const story = useStory( storyId ) as Story | null | undefined;
			...
			<VStack justify="top" className="newspack-story-budget__sidebar">
				...
				{ ( canEditStory || onCancel ) && (
					<HStack expanded direction="row-reverse" justify="end" style={ { padding: '16px', boxSizing: 'border-box' } }>
						...
						<Spacer />
						{ /* Pre-existing: unlike other reads of `story.metadata` above, this one doesn't optional-chain. */ }
