...
export interface BudgetAutoArchiveDateFieldProps {
	budget: Budget;
	onUpdateBudget?: ( budgetId: Budget[ 'id' ], updatedBudget: Budget ) => void;
}

type ArchiveDate = Budget[ 'archive_at' ];

const BudgetAutoArchiveDateField = ( { budget, onUpdateBudget = () => {} }: BudgetAutoArchiveDateFieldProps ) => {
	const [ isOpen, setIsOpen ] = useState( false );
	const [ autoArchiveDate, setAutoArchiveDate ] = useState< ArchiveDate | string >( budget.archive_at || '' );
	...
	const formatDate = ( dateString: ArchiveDate | string ) => {
		if ( ! dateString ) {
			return '';
		}
