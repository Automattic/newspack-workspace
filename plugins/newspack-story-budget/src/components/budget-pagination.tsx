...
const Pagination = ( { currentPage, totalPages, onPageChange }: PaginationProps ) => {
	// Extracted to a variable (rather than passed inline) so TypeScript's excess-property
	// check doesn't flag `CurrentPage`, which `createInterpolateElement`'s tag-extraction
	// can't see (it only inspects the static format string, not the `%1$s`-substituted result).
	const pageTextConversionMap = {
		div: <div aria-hidden />,
		CurrentPage: (
			<SelectControl
