...
describe( 'minifier', () => {
	...
	describe( 'minify and restore', () => {
		it( 'should minify using 36 base', () => {
			...
			// `minify()`'s `data` is typed `unknown` (its shape is whatever the caller minified);
			// this test's own input is a single flattened plain object, so it's cast to that shape.
			const data = minified.data as Record< string, unknown >[];
			expect( data[ 0 ][ 0 ] ).toEqual( 'value-0' );
			expect( data[ 0 ].a ).toEqual( 'value-10' );
			expect( data[ 0 ].b ).toEqual( 'value-11' );
			if ( ! minified.keyMap ) {
				throw new Error( 'Expected minify() to return a keyMap.' );
			}
	...
	it( 'should not minify arrays', () => {
		...
		expect( ( minified.data as Record< string, unknown > ).arr ).toEqual( [ 1, 2, 3 ] );
