...
import type { ErrorsState, StoreAction } from '../types';

export default ( state: ErrorsState = INITIAL_STATE.errors, action: StoreAction ): ErrorsState => {
	switch ( action.type ) {
		case 'SEARCH_ERROR':
		case 'STORIES_ERROR':
			return {
		...
		case 'SET_STORY_ERROR':
			// Dead code: no action creator dispatches this type (see store/types.ts).
			return {
				...state,
				storyError: action.payload as ErrorsState[ string ],
			};
		case 'SET_BUDGET_ERROR':
			// Dead code: no action creator dispatches this type (see store/types.ts).
			return {
				...state,
				budgetError: action.payload as ErrorsState[ string ],
			};
