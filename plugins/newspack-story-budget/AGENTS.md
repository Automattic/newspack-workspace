...
- **`getStories`/`getBudgets`/`getView` are `createSelector`-memoized with hand-written dependant arrays.** Reading new state inside the body without adding it to that array returns stale data with no build or lint error.
