...
/**
 * A brand term reference attached to a Campaigns prompt (added to the prompt
 * REST response by this plugin; not part of newspack-plugin's own prompt
 * type).
 */
interface PromptBrandTerm {
	term_id: number;
}

/**
 * The subset of a Campaigns prompt this filter reads: its assigned brands.
 */
interface PromptWithBrands {
	brand: PromptBrandTerm[];
}

addFilter(
	'newspack.wizards.campaigns.conflictingPrompts',
	'newspack/multibranded-site/brand-selector-filter',
	( conflicts: PromptWithBrands[], prompt: PromptWithBrands ) => {
