...
| `POST /newspack-blocks-posts-batch` | Same controller; runs each query through `/newspack-blocks-posts` and deduplicates in document order. The editor loads every Homepage Posts and Carousel block with it. | `edit_posts` |
