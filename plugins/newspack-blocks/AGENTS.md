...
| `POST /newspack-blocks-posts-batch` | Same controller; runs each query through `/newspack-blocks-posts` and deduplicates in document order. The editor uses it when two or more Homepage Posts or Carousel blocks need posts; a single block uses `/newspack-blocks-posts`. | `edit_posts` |
