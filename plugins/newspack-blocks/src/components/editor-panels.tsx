...
type CheckboxOption = { name: string; slug: string };

const CheckboxesGroup = ( {
	options,
	values,
	onChange,
	defaultRequiredSlug = null,
}: {
	options?: CheckboxOption[];
	values: string[];
	onChange: ( values: string[] ) => void;
	defaultRequiredSlug?: string | null;
} ) => {
	...
	return options.map( ( { name, slug } ) => {
		...
		const isDisabled = Boolean( isDefault && ! otherOptionsSelected );
...
export const PostTypesPanel = ( {
	...
	attributes: { postType: string[] };
	setAttributes: ( attributes: { postType: string[] } ) => void;
} ) => {
	const { availablePostTypes } = useSelect( select => {
		const { getPostTypes } = select( 'core' ) as {
			getPostTypes: ( query: { per_page: number } ) => PostType[] | undefined;
		};
...
export const PostStatusesPanel = ( {
	attributes,
	setAttributes,
}: {
	attributes: { includedPostStatuses: string[] };
	setAttributes: ( attributes: { includedPostStatuses: string[] } ) => void;
} ) => {
	return (
