...
type CategorySuggestion = { value: number; label: string };

class Edit extends Component< VideoPlaylistProps, VideoPlaylistState > {
	constructor( props: VideoPlaylistProps ) {
		super( props );
		this.state = {
			isLoading: false,
			error: '',
	 ...
	 *
	 * @param prevProps Previous object props.
	 */
	componentDidUpdate( prevProps: VideoPlaylistProps ) {
	 ...
	 * @param nextProps Next object props.
	 * @param state     Object state.
	 * @return State or null.
	 */
	static getDerivedStateFromProps( nextProps: VideoPlaylistProps, state: VideoPlaylistState ): Partial< VideoPlaylistState > | null {
	 ...
	 *
	 * @param search The text input into the field so far.
	 * @return Autocomplete suggestion tokens.
	 */
	fetchCategorySuggestions = ( search: string ) => {
		...
		} ).then( function ( categories ): CategorySuggestion[] {
	 ...
	 * @return Tokens.
	 */
	fetchSavedCategories = ( categoryIDs: number[] ) => {
		return apiFetch< Array< { id: number; name: string } > >( {
			path: addQueryArgs( '/wp/v2/categories', {
				per_page: 100,
				_fields: 'id,name',
...
const EditWithBlockProps = ( props: Omit< VideoPlaylistProps, 'blockProps' > ) => {
