...
if ( typeof registerBlockBindingsSource === 'function' ) {
	registerBlockBindingsSource( {
		...
		getValues( { bindings, clientId, select }: BlockBindingsGetValuesArgs ) {
			...
			const author: Author = ( parentId && authorMap[ parentId ] ) || {};
...
import type { Author, AuthorAttributes, AuthorSocialLink } from './single-author';
...
// Per-instance author map for block bindings, keyed by block clientId. Local to
// this block (not a workspace-wide global), so the augmentation lives here
// rather than in the shared src/types/ ambient declarations.
declare global {
	interface Window {
		__newspackAuthorsByBlock: Record< string, Author | null >;
	}
}

// A raw author suggestion, as returned by the `/authors` REST endpoint and
// consumed by the author-search autocomplete in this block and in author-list.
// Both `isGuestAuthor` (used here) and `isGuest` (used in author-list/edit.tsx)
// are included since the two blocks build the suggestion objects independently.
...
// Raw author suggestion shape returned by the /authors endpoint (fields=id,name).
type SuggestionResponse = { id: number | string; name: string; is_guest?: boolean };

// getValues() is declared as a loose `Function` in @wordpress/blocks'
// BlockBindingsSource type (registerBlockBindingsSource is an experimental,
// unstable API with no narrower upstream typing), so its param shape is
// re-typed here to match how WordPress core actually calls it.
type BlockBindingsGetValuesArgs = {
	bindings: Record< string, { args?: { key?: string } } >;
	clientId: string;
	select: ( store: string ) => { getBlockParents: ( clientId: string ) => string[] };
};

// BlockVariation.innerBlocks is loosely typed upstream as Array<unknown[]>;
// re-cast to the InnerBlocks-template shape createBlocksFromInnerBlocksTemplate()
// actually expects.
type InnerBlocksTemplateArg = Array< [ string, Record< string, unknown >?, unknown[]? ] >;

// This block's attributes (see block.json), plus the display-related attributes
// shared with author-list (AuthorAttributes).
type AuthorProfileAttributes = AuthorAttributes & {
	authorId: number;
	isGuestAuthor: boolean;
	isContextual: boolean;
	layoutVersion: number;
	avatarHideDefault?: boolean;
	showEmptyBio: boolean;
	variation?: string;
};
 ...
 */
type VariationPlaceholderProps = {
	clientId: string;
	name: string;
	setAttributes: ( attributes: Partial< AuthorProfileAttributes > ) => void;
};

function VariationPlaceholder( { clientId, name, setAttributes }: VariationPlaceholderProps ) {
	...
	return (
		<div { ...blockProps }>
			<BlockVariationPicker
				...
				onSelect={ ( nextVariation: BlockVariation = defaultVariation as BlockVariation ) => {
					setAttributes( { ...nextVariation.attributes, variation: nextVariation.name } );
					if ( nextVariation.innerBlocks ) {
						replaceInnerBlocks(
							clientId,
							createBlocksFromInnerBlocksTemplate( nextVariation.innerBlocks as InnerBlocksTemplateArg ),
							true
						);
					}
				} }
...
let socialIconSvgsCache: Promise< Record< string, string > > | null = null;
const fetchSocialIconSvgs = (): Promise< Record< string, string > > => {
	if ( ! socialIconSvgsCache ) {
		socialIconSvgsCache = apiFetch< Record< string, string > >( { path: '/newspack/v1/social-icons' } ).catch( () => ( {} ) );
	}
...
const DEFAULT_PLACEHOLDER_AUTHOR: Author = Object.freeze( {
	id: 'placeholder',
	url: '#',
...
const getPlaceholderAuthor = ( socialIconSvgs: Record< string, string > = {} ): Author => {
...
const AuthorProfile = ( { attributes, setAttributes, context, clientId }: AuthorProfileProps ) => {
	const { replaceInnerBlocks } = useDispatch( blockEditorStore );

	// ALL HOOKS MUST BE CALLED UNCONDITIONALLY (React rules of hooks)
	const [ author, setAuthor ] = useState< Author | null >( null );
	const [ contextualAuthors, setContextualAuthors ] = useState< Author[] >( [] );
	...
	const [ socialIconSvgs, setSocialIconSvgs ] = useState< Record< string, string > >( {} );
	...
	const editorPostId = useSelect(
		select => ( select( 'core/editor' ) as { getCurrentPostId: () => number } | undefined )?.getCurrentPostId?.(),
		[]
	);
		...
		select => {
			...
			const meta = (
				select( 'core/editor' ) as
					| { getEditedPostAttribute: ( attribute: string ) => { _newspack_byline_active?: boolean; _newspack_byline?: string } }
					| undefined
	...
	const isTemplateLikeContext = useSelect( select => {
		const postType = ( select( 'core/editor' ) as { getCurrentPostType: () => string } | undefined )?.getCurrentPostType?.();
	...
	const hasInnerBlocks = useSelect(
		select => !! ( select( 'core/block-editor' ) as { getBlocks: ( clientId?: string ) => unknown[] } ).getBlocks( clientId ).length,
		[ clientId ]
	);
	...
	useEffect( () => {
		if ( ! didAutoPopulate && variation && ! hasInnerBlocks && activeVariationTemplate ) {
			replaceInnerBlocks( clientId, createBlocksFromInnerBlocksTemplate( activeVariationTemplate as InnerBlocksTemplateArg ), true );
	...
	const getAuthorById = async () => {
		...
		setIsLoading( true );
		try {
			const params: { author_id: number; is_guest_author: number; fields: string; avatar_hide_default?: number } = {
				author_id: authorId,
	...
	const getContextualAuthors = async () => {
		...
		try {
			...
			const params: { post_id?: number; fields: string; avatar_hide_default?: number } = {
				post_id: postId,
				fields: fields.join( ',' ),
			};
			...
			setError( ( e as Error ).message || ( e as string ) || __( 'Error fetching authors for this post.', 'newspack-blocks' ) );
			setContextualAuthors( [] );
		}
		setIsLoading( false );
	};
	...
	const getBylineAuthors = async () => {
		...
		try {
			...
			setError( ( e as Error ).message || ( e as string ) || __( 'Error fetching byline authors.', 'newspack-blocks' ) );
			setContextualAuthors( [] );
		}
		setIsLoading( false );
	};
	...
	const getSocialLinks = ( authorData: Author ): Record< string, AuthorSocialLink > => {
		const socialLinks: Record< string, AuthorSocialLink > = ( showSocial && authorData?.social ) || {};
			...
			{ ! isNestedLayout && (
				<PanelBody title={ __( 'Settings', 'newspack-blocks' ) }>
					<ToggleGroupControl
						...
						value={ textSize }
						onChange={ value => setAttributes( { textSize: value as string } ) }
						isBlock
				...
				<PanelBody title={ __( 'Avatar', 'newspack-blocks' ) }>
					...
					{ showAvatar && (
						<ToggleGroupControl
							...
							value={ avatarSize }
							onChange={ value => setAttributes( { avatarSize: value as number } ) }
							isBlock
							__next40pxDefaultSize
						>
					...
					{ showAvatar && (
						<UnitControl
							...
							onChange={ value => setAttributes( { avatarBorderRadius: 0 > parseFloat( value as string ) ? '0' : value } ) }
						/>
					) }
				</PanelBody>
			) }
				...
				<div { ...blockProps }>
					...
					<Placeholder className="newspack-blocks-author-profile" icon={ postAuthor } label={ __( 'Author Profile', 'newspack-blocks' ) }>
						...
						<AutocompleteWithSuggestions
							...
							fetchSuggestions={ async ( search: string | null = null, offset = 0 ) => {
								setSuggestions( null );
								if ( authorId && ! error ) {
									return [];
								}
								...
								const total = parseInt( ( response.headers.get( 'x-wp-total' ) || 0 ) as string, 10 );
								const authors: SuggestionResponse[] = await response.json();
							...
							onChange={ ( items: AuthorSuggestion[] ) => {
								let selectionIsGuest = false;
								const selection = items[ 0 ];
								if ( suggestions ) {
									suggestions.forEach( suggestion => {
										if (
											parseInt( String( selection?.value ), 10 ) === parseInt( String( suggestion?.value ), 10 ) &&
								...
								setAttributes( {
									authorId: parseInt( String( selection?.value || 0 ), 10 ),
