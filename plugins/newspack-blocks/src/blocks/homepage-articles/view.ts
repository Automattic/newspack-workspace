...
// "Load more" fetch-queueing state, local to this view script (not a
// workspace-wide global), so the augmentation lives here rather than in the
// shared src/types/ ambient declarations.
declare global {
	interface Window {
		newspackBlocksIsFetching: boolean;
		newspackBlocksFetchQueue: Array< () => void >;
	}
}

// The "Load more" posts endpoint payload shape (see isPostsDataValid's JSDoc below).
...
function buildLoadMoreHandler( blockWrapperEl: HTMLElement ) {
	...
	const postsContainerEl = blockWrapperEl.querySelector( '[data-posts]' ) as Element;
	...
	function onEnd() {
		...
		if ( window.newspackBlocksFetchQueue.length ) {
			// The length check above guarantees shift() returns an element here.
...
function getRenderedPostsIds() {
	...
	postIds.push( ( document.querySelector( 'div[data-current-post-id]' ) as Element ).getAttribute( 'data-current-post-id' ) );
 ...
 * @param {Function} options.onSuccess Called with the parsed JSON response on success.
 * @param {Function} options.onError   Called once retries are exhausted.
 * @param {number}   n                 retry count before throwing
 */
function fetchWithRetry( options: { url: string; onSuccess: ( data: unknown ) => void; onError: () => void }, n: number ) {
 ...
 */
function isPostsDataValid( data: unknown ): data is LoadMoreResponse {
	let isValid = false;

	// NOTE: pre-existing behavior, not changed here: the schema above documents
	// `next` as `string | null`, but this check requires `typeof === 'string'`,
	// so a legitimate `next: null` "end of pagination" response is treated as
	// invalid data (routed to onError()) rather than as a valid final page.
	if (
		data &&
		...
		if ( items.length && ! ( hasOwnProp( items[ 0 ], 'html' ) && typeof ( items[ 0 ] as { html: unknown } ).html === 'string' ) ) {
			isValid = false;
		}
	}
