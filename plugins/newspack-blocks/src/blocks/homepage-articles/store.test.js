...
// Stands in for the batch endpoint: every query draws from posts 1–20, newest first, and
// deduplicating queries skip the posts already shown above them.
const fakeBatchEndpoint = ( { data } ) => {
	...
	return Promise.resolve(
		data.queries.map( ( { clientId, postsQuery, deduplicate } ) => {
			...
			const ids = [];
			for ( let id = 1; id <= 20 && ids.length < postsQuery.postsToShow; id++ ) {
				if ( ! deduplicate || ! exclude.includes( id ) ) {
					ids.push( id );
				}
			}
			if ( deduplicate ) {
...
describe( 'fetchPostsForBlocks', () => {
	...
	beforeEach( () => {
		window.newspack_blocks_data = { posts_batch_rest_url: BATCH_URL, posts_batch_max_queries: 50 };
		jest.isolateModules( () => {
			apiFetch = require( '@wordpress/api-fetch' ).default;
			( { fetchPostsForBlocks } = require( './store' ) );
		} );
		apiFetch.mockImplementation( fakeBatchEndpoint );
	} );
	...
	it( 'reports an error on every remaining block when the request fails', async () => {
		window.newspack_blocks_data.posts_batch_max_queries = 1;
		apiFetch.mockImplementationOnce( fakeBatchEndpoint ).mockRejectedValueOnce( new Error( 'Service unavailable' ) );

		const { postsByBlock, errorsByBlock } = await run( [ block( 'a', 1 ), block( 'b', 1 ), block( 'c', 1 ) ] );

		expect( postsByBlock ).toEqual( { a: [ 1 ] } );
		expect( errorsByBlock ).toEqual( { b: 'Service unavailable', c: 'Service unavailable' } );
		expect( apiFetch ).toHaveBeenCalledTimes( 2 );
	} );
	...
	it( 'splits a page larger than the batch limit and carries deduplication across batches', async () => {
		...
		const { postsByBlock } = await run( [ block( 'a', 1 ), block( 'b', 1 ), block( 'c', 1 ) ] );

		expect( apiFetch ).toHaveBeenCalledTimes( 2 );
		expect( apiFetch.mock.calls[ 1 ][ 0 ].data.exclude ).toEqual( [ 1, 2 ] );
		expect( postsByBlock ).toEqual( { a: [ 1 ], b: [ 2 ], c: [ 3 ] } );
	} );
} );
