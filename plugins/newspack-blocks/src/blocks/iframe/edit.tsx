...
// `FocusableIframeProps` declares `iframeRef` as required, but the component
// itself merges it via `useMergeRefs`, which tolerates it being omitted - this
// usage has never passed a ref. Re-type it at this boundary without altering
// the rendered markup.
const Iframe = FocusableIframe as import('react').FC< import('react').ComponentPropsWithoutRef< 'iframe' > >;
...
type IframeEditProps = {
	...
	setAttributes: ( attributes: Partial< IframeAttributes > ) => void;
};

const IframeEdit = ( { attributes, setAttributes }: IframeEditProps ) => {
	const blockProps = useBlockProps();
	const label = __( 'Iframe', 'block title' );
	const { mode, src, archiveFolder, isFullScreen, height, width } = attributes;
	const [ showPreview, setShowPreview ] = useState( true );
	const [ isUploadingArchive, setIsUploadingArchive ] = useState< boolean >();
	...
	const embedURL = async ( url: string ) => {
		...
		try {
			...
			const { mode: iframeMode } = await apiFetch< { mode: string } >( {
			...
			setError( ( e as { message?: string } ).message || __( 'An error occured when uploading the iframe archive.', 'newspack-blocks' ) );
		}

		setIsUploadingArchive( false );
	};

	const uploadIframeArchive = async ( archiveFile: File ) => {
		setError( null );
		setIsUploadingArchive( true );
		...
		try {
			...
			const {
				...
				mode: iframeMode,
			} = await apiFetch< { src: string; dir: string; mode: string } >( {
	...
	const setIframeArchiveFromMedia = async ( mediaId: string ) => {
		setError( null );
		setIsUploadingArchive( true );
		...
		try {
			...
			setError(
				( e as { message?: string } ).message || __( 'An error occured when setting the iframe from the archive media.', 'newspack-blocks' )
			);
		}
	...
	return (
		<Fragment>
			<BlockControls>
				<Toolbar controls={ ( src && iframeControls ) as LegacyToolbarProps[ 'controls' ] } />
			</BlockControls>
