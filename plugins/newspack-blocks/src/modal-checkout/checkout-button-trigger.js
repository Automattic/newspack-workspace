...
/**
 * Selector for a picker card's price, as rendered by
 * Newspack\Subscriptions_Tiers::render_product_card().
 *
 * @type {string}
 */
const PICKER_PRICE_SELECTOR = '.newspack__subscription-tiers__price';

/** Attribute holding a price node's undiscounted markup, so it can be restored. */
const ORIGINAL_PRICE_ATTR = 'data-newspack-price';

/** Class marking a price node the coupon annotation has rewritten. */
const DISCOUNTED_CLASS = 'newspack-blocks__picker-price--discounted';

/**
 * Find a variation's price node inside a picker form.
 *
 * @param {HTMLFormElement} targetForm  Picker form.
 * @param {string}          variationId Variation (radio) value.
 *
 * @return {HTMLElement|null} The price node, or null.
 */
function findPickerPriceNode( targetForm, variationId ) {
	const radio = targetForm.querySelector( `input[name="product_id"][value="${ variationId }"]` );
	const card = radio ? radio.closest( 'label' ) : null;
	return card ? card.querySelector( PICKER_PRICE_SELECTOR ) : null;
}

/**
 * Restore every price the coupon annotation rewrote.
 *
 * The picker is shared and never re-rendered, so this has to run before each
 * annotation — and on any open whose button carries no coupon, or the previous
 * button's discount would stay on screen.
 *
 * @param {HTMLFormElement|null} targetForm Picker form.
 *
 * @return {void}
 */
export function clearPickerPricing( targetForm ) {
	if ( ! targetForm ) {
		return;
	}
	targetForm.querySelectorAll( `[${ ORIGINAL_PRICE_ATTR }]` ).forEach( node => {
		node.innerHTML = node.getAttribute( ORIGINAL_PRICE_ATTR );
		node.removeAttribute( ORIGINAL_PRICE_ATTR );
		node.classList.remove( DISCOUNTED_CLASS );
	} );
}

/**
 * Show an attached coupon's effect on each variation's price.
 *
 * Every string rendered here is server-built (wc_price / wcs_price_string, and a
 * translated qualifier), so no copy is assembled in the browser.
 *
 * @param {HTMLFormElement|null} targetForm Picker form.
 * @param {Object|null}          pricing    Response from the coupon-pricing route.
 *
 * @return {void}
 */
export function applyPickerPricing( targetForm, pricing ) {
	// Always clear first: an open with no discount must undo the previous one.
	clearPickerPricing( targetForm );
	if ( ! targetForm || ! pricing || ! pricing.applies || ! pricing.variations ) {
		return;
	}
	const doc = targetForm.ownerDocument;
	Object.keys( pricing.variations ).forEach( variationId => {
		const variation = pricing.variations[ variationId ];
		if ( ! variation || ! variation.applies || ! variation.first_html ) {
			return;
		}
		const node = findPickerPriceNode( targetForm, variationId );
		if ( ! node ) {
			return;
		}
		node.setAttribute( ORIGINAL_PRICE_ATTR, node.innerHTML );
		node.classList.add( DISCOUNTED_CLASS );

		const was = doc.createElement( 's' );
		was.className = 'newspack-blocks__picker-price__was';
		was.innerHTML = variation.regular_html;
		const now = doc.createElement( 'span' );
		now.className = 'newspack-blocks__picker-price__now';
		now.innerHTML = variation.first_html;

		node.textContent = '';
		node.append( was, doc.createTextNode( ' ' ), now );

		if ( variation.recurring_html ) {
			const then = doc.createElement( 'span' );
			then.className = 'newspack-blocks__picker-price__then';
			then.innerHTML = variation.recurring_html;
			node.append( then );
		}
	} );
}
