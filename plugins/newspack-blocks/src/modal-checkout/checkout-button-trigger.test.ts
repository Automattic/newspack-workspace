 ...
 *
 * @param checkoutData Object to JSON-encode into data-checkout, or null to omit it.
 * @param label        Button label.
 * @return HTML.
 */
const checkoutButton = ( checkoutData: Record< string, unknown > | null, label = 'Buy' ): string => {
 ...
 *
 * @param productId Parent product id for the picker container.
 * @param radioIds  Radio values (variation/child ids).
 * @return HTML.
 */
const variationPicker = ( productId: string, radioIds: string[] ): string => {
...
describe( 'selectPickerForm', () => {
	it( 'checks the radio matching the variation and returns the picker form', () => {
		...
		expect( root.querySelector< HTMLInputElement >( 'input[value="1407"]' )!.checked ).toBe( true );
		expect( root.querySelector< HTMLInputElement >( 'input[value="1408"]' )!.checked ).toBe( false );
	} );
	...
	it( 'only selects radio inputs inside the checkout iframe form', () => {
		...
		const checkoutForm = root.querySelector< HTMLFormElement >( `form[target="${ IFRAME_NAME }"]` );

		expect( selectPickerForm( root, '1406', '1407', PICKER_OPTIONS ) ).toBeNull();
		expect( selectPickerForm( root, '1406', '1408', PICKER_OPTIONS ) ).toBe( checkoutForm );
		expect( checkoutForm!.querySelector< HTMLInputElement >( 'input[type="radio"][value="1408"]' )!.checked ).toBe( true );
	} );
} );
...
describe( 'resolveCheckoutButtonForm', () => {
	...
	it( 'drives the grouped picker without throwing on its data-checkout-less form', () => {
		...
		expect( result ).toBe( pickerForm );
		expect( root.querySelector< HTMLInputElement >( 'input[value="158"]' )!.checked ).toBe( true );
	} );
	...
	it( 'copies block context from the source button into the picker form without submitting the locked button', () => {
		...
		expect( pickerForm!.querySelector< HTMLInputElement >( 'input[name="after_success_button_label"]' )!.value ).toBe( 'Thanks!' );
		expect( pickerForm!.querySelector< HTMLInputElement >( 'input[name="after_success_url"]' )!.value ).toBe( '/welcome/' );
	} );
} );
...
describe( 'copyContextFields', () => {
	it( 'copies present source fields, skips missing ones, and does not overwrite existing target fields', () => {
		...
		const source = root.querySelector< HTMLFormElement >( '#src' );
		const target = root.querySelector< HTMLFormElement >( '#dst' );

		copyContextFields( source, target );

		// Copied from source.
		expect( target!.querySelector< HTMLInputElement >( 'input[name="after_success_url"]' )!.value ).toBe( '/welcome/' );
		// Not overwritten.
		expect( target!.querySelector< HTMLInputElement >( 'input[name="prompt_title"]' )!.value ).toBe( 'Existing' );
	...
	it( 'copies the last source value when a context field is duplicated', () => {
		...
		expect( target!.querySelector< HTMLInputElement >( 'input[name="after_success_url"]' )!.value ).toBe( '/last/' );
	} );
