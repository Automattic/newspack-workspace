...
class Starter_Content_Generated extends Starter_Content_Provider {
	...
	public static function generate_title( $post_index = 0 ) {
		if ( Starter_Content::is_e2e() ) {
			return $post_index . ' ' . file_get_contents( NEWSPACK_ABSPATH . 'includes/raw_assets/markup/title.txt' ); // phpcs:ignore WordPressVIPMinimum.Performance.FetchingRemoteData.FileGetContentsUnknown -- reads an asset shipped inside the plugin, never a remote URL.
		}
	...
	public static function get_lipsum( $type, $amount ) {
		if ( Starter_Content::is_e2e() ) {
			return file_get_contents( NEWSPACK_ABSPATH . 'includes/raw_assets/markup/body.txt' ); // phpcs:ignore WordPressVIPMinimum.Performance.FetchingRemoteData.FileGetContentsUnknown -- reads an asset shipped inside the plugin, never a remote URL.
		}
		if ( 'paras' === $type ) {
