...
class Group_Subscription_Teams_Invite {
	...
	public static function resolve_registration_token( $token ) {
		...
		if ( empty( $entry['key'] ) ) {
			// Only ever minted when the owner has no link at all: generate_link_invite()
			// replaces whatever that manager has stored, so calling it on an owner who
			// is already circulating a link would revoke it from under them.
