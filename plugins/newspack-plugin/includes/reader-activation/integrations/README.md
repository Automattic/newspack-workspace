...
3. Optionally override `handle_logged_in_user_registration( $user, $request )` to react when an already-logged-in reader submits a registration request — record a new donation, link an account, fire an analytics event, etc. The handler runs only behind the endpoint's gates: a rejection by the integration's own `validate_registration_request()`, the rate limit, or reCAPTCHA suppresses it. `npe` stays caller-supplied on the logged-in path (it is not the session email), so a key scheme that binds to it is not a session check — keep the validator's requirements no stricter than what the logged-in handler itself needs.
