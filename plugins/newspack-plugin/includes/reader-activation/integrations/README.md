...
| `outgoing_metadata_fields` | outbound | `metadata` | Subset of Newspack metadata fields to push. Stored at `newspack_integration_outgoing_fields_{id}`. When the option was never stored, the integration defaults to every available field (matching the pre-coexistence passthrough behavior); a stored empty selection means none. |
