...
describe( 'pricing helpers', () => {
	it( 'computes fixed and percent member prices, never negative', () => {
		expect( memberPrice( 450, { type: 'fixed', amount: 51 } ) ).toBe( 399 );
		expect( memberPrice( 100, { type: 'percent', amount: 10 } ) ).toBe( 90 );
		expect( memberPrice( 30, { type: 'fixed', amount: 51 } ) ).toBe( 0 );
	} );
	...
	it( 'labels discounts and targeting', () => {
		expect( discountLabel( { type: 'percent', amount: 10 } ) ).toBe( '10% off' );
