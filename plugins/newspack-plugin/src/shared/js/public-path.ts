...
if ( typeof window === 'object' && window.newspack_urls ) {
	// `newspack_urls` is a PHP-localized payload; `public_path` is not part of the
	// NewspackUrls shape declared by newspack-components, hence the assertion.
	// The global is write-only from TS's perspective (webpack reads it), so it also
	// trips no-unused-vars on assignment.
	// eslint-disable-next-line no-global-assign, @typescript-eslint/no-unused-vars
	__webpack_public_path__ = ( window.newspack_urls as { public_path: string } ).public_path;
}
