 ...
 */
export const handleSegmentation = ( prompts: PromptElement[] ): void => {
	const maybeDisplayPrompts = ( ras: NewspackReaderActivation | null = null ) => {
		...
		let overlayDisplayed: boolean | undefined;
		...
		prompts.forEach( prompt => {
			...
			if ( shouldDisplay ) {
				const delayPrompt = () => {
					...
					// `Number( delay )` mirrors the implicit-to-number coercion `setTimeout()`
					// already performs at runtime on a string delay argument.
					setTimeout( unhide, Number( delay ) );
				};
				const unhide = () => {
