...
interface PositionPlacementControlProps {
	layout: string;
	label: string;
	help: string;
	onChange: ( value: string ) => void;
	size: string;
	/** Passed through by `Sidebar` but not destructured here -- forwarded via `...props` onto `ButtonGroup`. */
	value?: string;
}

const PositionPlacementControl = ( { layout, label, help, onChange, size, ...props }: PositionPlacementControlProps ) => {
	/**
	 * Set layout options
	 */
	const layoutOptions =
				...
				{ layoutOptions.map( ( { label: layoutLabel, value }, index ) => {
					return (
						<Tooltip text={ layoutLabel } key={ `grid-tooltip-${ index }` }>
							<div className={ value === layout ? 'is-selected' : undefined }>
								<Button
									isSmall
