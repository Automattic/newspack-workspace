...
const Sidebar = ( props: SidebarProps ) => {
	...
	const updatePlacement = ( value: string ) => {
	...
	// Pre-existing bug (not introduced by this migration, left as-is): the fallback is `{}`
	// though `popup_size_options` is an array and `SelectControl` expects an options array;
	// only reachable if `newspack_popups_data` is unset, which shouldn't happen on this screen.
	const popupSizeOptions = window.newspack_popups_data?.popup_size_options || ( {} as NewspackPopupsSizeOption[] );
