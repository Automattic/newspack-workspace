...
import { SVG, ToolbarGroup, ToolbarDropdownMenu } from '@wordpress/components';

/**
 * A block's text selection endpoint, as returned by the block-editor store's
...
/**
 * The subset of the `core/block-editor` store's selectors/actions this
 * component calls. `@wordpress/data` ships no usable types for the
 * string-keyed store registry, so the shape is re-declared here at the call site.
 */
interface BlockEditorSelectors {
	getSelectedBlockClientId: () => string | null;
	getSelectionStart: () => BlockSelectionPoint;
	getSelectionEnd: () => BlockSelectionPoint;
}

interface BlockEditorActions {
	selectionChange: ( clientId: string, attributeKey: string, start: number, end: number ) => void;
}

interface MergeTagsBlockControlProps {
...
const MergeTagsBlockControl = ( { tags, attributes, setAttributes }: MergeTagsBlockControlProps ) => {
	const { clientId, selectionStart, selectionEnd } = useSelect( select => {
		const blockEditorSelect = select( 'core/block-editor' ) as BlockEditorSelectors;
		return {
			clientId: blockEditorSelect.getSelectedBlockClientId(),
			selectionStart: blockEditorSelect.getSelectionStart(),
			selectionEnd: blockEditorSelect.getSelectionEnd(),
		};
	} );

	const { selectionChange } = useDispatch( 'core/block-editor' ) as BlockEditorActions;
