...
type AdvancedSidebarProps = Pick< PromptEditorProps, 'onMetaFieldChange' | 'excluded_categories' | 'excluded_tags' | 'additional_classes' >;

const AdvancedSidebar = ( { onMetaFieldChange, excluded_categories = [], excluded_tags = [], additional_classes = '' }: AdvancedSidebarProps ) => {
	return (
		<Fragment>
			<BaseControl>
				<TextControl
				...
				<CategoryAutocomplete
					...
					onChange={ tokens =>
						onMetaFieldChange( {
							excluded_categories: tokens.map( token => parseInt( String( token.id ) ) ),
						} )
					}
				/>
				<hr />
				<CategoryAutocomplete
					...
					onChange={ tokens =>
						onMetaFieldChange( {
							excluded_tags: tokens.map( token => parseInt( String( token.id ) ) ),
						} )
					}
				/>
			</BaseControl>
