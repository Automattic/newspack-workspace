 ...
 */
import type { PromptEditorProps, PromptMeta } from './utils';

type FrequencySidebarProps = Pick<
	PromptEditorProps,
	'frequency' | 'frequency_max' | 'frequency_start' | 'frequency_between' | 'frequency_reset' | 'onMetaFieldChange' | 'utm_suppression'
>;
		...
		<Fragment>
			<SelectControl
				...
				onChange={ ( value: string ) => {
					const metaToUpdate: Partial< PromptMeta > = {
						frequency: value,
					};
			...
			{ 'custom' === frequency && (
				<>
					<NumberControl
						...
						min={ 1 }
						onChange={ ( value: string | undefined ) => onMetaFieldChange( { frequency_start: Number( value ) - 1 } ) }
					/>
					<NumberControl
						className="newspack-popups__frequency-number-control"
						...
						min={ 1 }
						onChange={ ( value: string | undefined ) => onMetaFieldChange( { frequency_between: Number( value ) - 1 } ) }
					/>
					<ToggleControl
						...
						onChange={ ( value: boolean ) => onMetaFieldChange( { frequency_max: value ? 1 : 0 } ) }
					/>
					{ 0 < frequency_max && (
						<NumberControl
							className="newspack-popups__frequency-number-control"
							...
							value={ frequency_max }
							min={ 0 }
							onChange={ ( value: string | undefined ) => onMetaFieldChange( { frequency_max: Number( value ) } ) }
						/>
					) }
					<SelectControl< string >
						...
						onChange={ ( value: string ) => onMetaFieldChange( { frequency_reset: value } ) }
					/>
				</>
			) }
			<hr />
			<TextControl
				...
				onChange={ ( value: string ) => onMetaFieldChange( { utm_suppression: value } ) }
			/>
		</Fragment>
	);
};
