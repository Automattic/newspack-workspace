...
// `ras` is only ever called for a registered criteria's matching attribute
// once the reader-activation client has loaded (see `getValue()`/`setup()` in
// `../utils`); the non-null assertion mirrors that pre-existing assumption
// rather than adding a new guard.
