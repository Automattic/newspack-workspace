...
import type { Suggestion, TokenValue } from '../autocomplete-tokenfield';
import './style.scss';

/**
 * A selected item: a suggestion, optionally tagged with its post type.
 */
export type SelectedItem = Suggestion & { postType?: string };
...
type AutocompleteWithSuggestionsProps = {
	...
	/** Function to call when the post type selector is changed. */
...
const AutocompleteWithSuggestions = ( {
	 ...
	 */
	const handleFetchSaved: FetchSavedPosts = fetchSavedPosts
		? fetchSavedPosts
		: async ( postIds = [], searchSlug = null ) => {
				...
				const posts = await apiFetch< { id: number | string; title: string }[] >( {
	...
	const handleFetchSuggestions: FetchSuggestions = fetchSuggestions
		...
		: async ( search = null, offset = 0, searchSlug = null ) => {
				...
				return posts.reduce( ( acc: Suggestion[], post ) => {
					acc.push( {
						value: parseInt( `${ post.id }` ),
	 ...
	 */
	const handleOnChange = ( _selectionsRaw: ( string | number | undefined )[] | Suggestion[] ) => {
		// This component always uses returnFullObjects, so selections arrive as value/label pairs.
		const _selections = _selectionsRaw as Suggestion[];
	...
	const renderPostTypeSelect = () => {
		return (
			<SelectControl
				...
				onChange={ ( _postType: string ) => setPostTypeToSearch( _postType ) }
			/>
		);
	};
	...
	return (
		<div className={ classNames.join( ' ' ) }>
			...
			<AutocompleteTokenField
				...
				fetchSuggestions={ async ( search: string ) => handleFetchSuggestions( search, 0, postTypeToSearch ) }
				fetchSavedInfo={ ( postIds: TokenValue[] ) => handleFetchSaved( postIds ) }
				label={ label }
				loading={ isLoading }
				help={ ! hideHelp && help }
				returnFullObjects
