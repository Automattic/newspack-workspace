...
/**
 * Plugin data fetched from the Newspack plugins API, keyed by original
 * (PascalCase) field names.
 */
type PluginApiData = Record< string, unknown > & {
	Status?: string;
	error?: string;
	inFlight?: string;
};
...
/**
 * Per-plugin configuration provided via the `plugins` prop, merged over the
 * camelCased API data before rendering.
 */
export interface PluginToggleConfig {
	/** Human-readable name override. */
	name?: string;
	/** Description override. */
	description?: string;
	/** URL the card's action links to. */
	href?: string;
	/** Path to the plugin's settings screen, used for handoff. */
	editPath?: string;
...
/**
 * A plugin's data prepared for rendering: camelCased API data merged with the
 * plugin's configuration.
 */
type PreparedPlugin = PluginToggleConfig & {
	slug?: string;
	status?: string;
	inFlight?: string;
	[ key: string ]: unknown;
...
type PluginToggleProps = {
	/** Configuration of the managed plugins, keyed by plugin slug. */
	plugins: Record< string, PluginToggleConfig >;
};

type PluginToggleState = {
	pluginInfo: Record< string, PluginApiData >;
};

/**
 * Plugin toggle group component.
 */
class PluginToggle extends Component< PluginToggleProps, PluginToggleState > {
	...
	prepareDataForRender = (
		...
		Object.keys( pluginsFromProps )
			...
			.map( ( plugin: PreparedPlugin ) => Object.assign( plugin, pluginsFromProps[ String( plugin.slug ) ] ) );
	...
	errorForPlugin = ( plugin: PreparedPlugin ) => {
		...
		return pluginInfo[ String( slug ) ]?.error || null;
	};
