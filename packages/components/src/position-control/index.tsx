...
type PositionControlProps = {
	/** Positions the user is allowed to pick. All positions are allowed when empty. */
...
export default function PositionControl( { allowedPositions, value, label, help, onChange, size, ...props }: PositionControlProps ) {
	/**
	 * Set layout options
	 */
	const options =
		...
		<div className={ classnames( 'newspack-position-placement-control', 'size-' + size ) }>
			...
			<ButtonGroup aria-label={ __( 'Select Position', 'newspack-plugin' ) } { ...props }>
				{ options.map( ( option, index ) => {
					return (
						<div key={ `newspack-position-placement-item-${ index }` } className={ option.value === value ? 'is-selected' : undefined }>
							<Button
								isSmall
								title={ option.label }
