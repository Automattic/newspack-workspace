...
export interface BreadcrumbItem {
	/** Crumb label. */
	label?: ReactNode;
	/** URL the crumb links to; the current-page crumb carries none. */
	url?: string;
	/** Item count annotating the crumb. */
	count?: number;
	/** Translated accessible phrasing for the count. */
	countLabel?: string;
}
 ...
 */
const CrumbCount = ( { count, countLabel }: { count?: number; countLabel?: string } ) => {
			...
			<span className="screen-reader-text">
				{ countLabel ||
					sprintf(
						...
						_n( '%s item', '%s items', value, 'newspack-plugin' ),
						formatted
					) }
			</span>
		</span>
 ...
 */
const CrumbLabel = ( { label, count, countLabel }: BreadcrumbItem ): ReactNode => {
	if ( ! Number.isFinite( count ) ) {
		return label;
	}
	return (
