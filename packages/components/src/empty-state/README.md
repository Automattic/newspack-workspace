...
`className` lands on the grid rather than a wrapper, because consumers key off
it. The newsletters shell hides its header and constrains its main region with
`:has( .newspack-newsletters-admin__empty-state )`.
...
A centred row. With one action, prefer a single primary button: an empty state
asking for two decisions at once is usually a sign the screen needs an
onboarding view instead.
