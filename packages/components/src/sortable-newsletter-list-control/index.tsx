...
export interface NewsletterList {
	...
	/** The list's name. */
	name: string;
	/** Whether the list is available for selection. */
	active?: boolean;
}

export interface SelectedNewsletterList {
	/** The selected list's ID. */
	id: string | number;
	/** Whether the list is checked by default. */
	checked?: boolean;
}

type SortableNewsletterListControlProps = {
	/** The available lists, or an error map keyed by error code. */
	lists: NewsletterList[] | { errors: Record< string, string > };
...
export default function SortableNewsletterListControl( { lists, selected = [], onChange = () => {} }: SortableNewsletterListControlProps ) {
	...
	// Past the error guard the lists are the available-lists array.
	const availableLists = lists as NewsletterList[];
	const getList = ( id: string | number ) => availableLists.find( list => list.id === id );
	const getAvailableLists = () => {
		return availableLists.filter( list => list.active && ! selected.find( ( { id } ) => id === list.id ) );
	};
	return (
