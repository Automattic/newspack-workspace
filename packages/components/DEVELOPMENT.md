...
- **`StatCard`** – Use for a single headline figure with a label above it and a description below. Compound: `StatCard.Root` wraps `Label`, `Body` (holding `Value` and an optional `Secondary`) and `Footer`. Format the figure yourself and pass a string; `value={ null }` gets the shared null glyph rather than a misleading zero. Sized by a container query, so a row of cards keeps one type scale. See [src/stat-card/README.md](src/stat-card/README.md).
